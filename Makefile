...
SOURCES = smbandpass~.c smcos~.c smenvg~.c smhighpass~.c smintg~.c \
	  smitrain~.c smkey~.c smlimit~.c smlowpass~.c smnoise~.c smnotch~.c \
	  smverb~.c
...
SHARED_SOURCES ?= $(wildcard lib$(LIBRARY_NAME).c)
SHARED_HEADERS ?= $(shell test ! -e $(LIBRARY_NAME).h || echo $(LIBRARY_NAME).h)
SHARED_LIB ?= $(wildcard lib$(LIBRARY_NAME).$(SHARED_EXTENSION))
...
TAGS: $(wildcard $(PD_INCLUDE)/*.h) $(SOURCES) $(SHARED_SOURCES) $(SHARED_HEADERS)
	etags $(wildcard $(PD_INCLUDE)/*.h)
	etags -a *.h $(SOURCES) $(SHARED_SOURCES) $(SHARED_HEADERS)
